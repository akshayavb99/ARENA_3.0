...
# %%
arr8 = einops.reduce(
    einops.rearrange(arr, '(b1 b2) c h w -> c (b1 h) (b2 w)', b1=2, b2=3),
    'c (h h2) (w w2) -> c h w',
    'max',
    h2=2,
    w2=2
...
def rearrange_1() -> Tensor:
    ...
    [[3, 4],
     ...
     [7, 8]]
    """
    print(einops.rearrange(t.arange(3,9), '(r c) -> r c', r=3))
...
def temperatures_differences(temps: Tensor) -> Tensor:
    ...
    """
    assert len(temps) % 7 == 0
    avg_temps = einops.repeat(einops.reduce(temps, '(w 7) -> w', "mean"), 'w -> (w 7)')
    #print(avg_temps)
...
def temperatures_normalized(temps: Tensor) -> Tensor:
    ...
    """
    avg_temps = einops.repeat(einops.reduce(temps, '(w 7) -> w', "mean"), 'w -> (w 7)')
    std_temps = einops.repeat(einops.reduce(temps, "(h 7) -> h", t.std), 'w -> (w 7)')
    #print(std_temps)
    return (temps - avg_temps) / std_temps
...
def cos_sim_matrix(matrix: Tensor) -> Tensor:
    ...
    matrix: shape (m, n)
    """
    norm_rows = normalize_rows(matrix)
    result = t.matmul(norm_rows, norm_rows.T)
    return result
...
def sample_distribution(probs: Tensor, n: int) -> Tensor:
    ...
    """
    probs_cumsum = t.cumsum(probs, dim=0)
    return (t.rand(n,1) > probs_cumsum).sum(dim=-1)
...
def total_price_indexing(prices: Tensor, items: Tensor) -> float:
    ...
    """
    return t.sum(t.gather(prices, 0, items))
